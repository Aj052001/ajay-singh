...
import { resumeLink } from "@/lib/constant";
...
export default function Navbar() {
  ...
  return (
    <div
      className={cn(
        "fixed left-0 top-0 w-full z-50 flex justify-center  md:pt-0 transition-all duration-300"
      )}
    >
      <div
      ...
      >
        <nav
          className={cn(
            "relative w-full rounded-1xl bg-black/40 backdrop-blur-xl border border-white/10 md:transition-all md:duration-300 transition-opacity duration-300 ease-in-out transform",
            {
              "opacity-0 translate-y-[-100%]":
                !isNavbarVisible && windowWidth < 768, // Hide navbar with transition
              "opacity-100 translate-y-0":  windowWidth < 768, // Show navbar
            }
          )}
          style={
            windowWidth >= 768
      ...
      />
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="hidden md:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
                <Link
                  href="#about"
                  className="text-[#989898] hover:text-white transition-colors text-xl"
                >
                  About
                </Link>
                <Link
            ...
            <div className="hidden md:flex items-center gap-2">
              <div className="flex items-center gap-1">
                <Link
                  href="https://leetcode.com/u/Ajay032001/"
                  target="_blank"
                  className="group relative p-2 transition-all duration-300"
                >
                  <div className="absolute -inset-2 bg-gradient-to-r from-[#7e7a73] via-[#B3B3B3] to-[#a8a39b] rounded-lg opacity-0 group-hover:opacity-75 transition-opacity duration-300 blur" />
                ...
                <Link
                  ...
                  className="group relative p-2 transition-all duration-300"
                >
                  <div className="absolute -inset-2 bg-gradient-to-r from-[#0A66C2] via-[#5e5656] to-[#0A66C2] rounded-lg opacity-0 group-hover:opacity-75 transition-opacity duration-300 blur" />
              ...
              <a href={resumeLink} target="_blank">
                <Button className="w-full bg-primary hover:bg-primary/90 text-black transition-all relative group overflow-hidden rounded-lg">
            ...
            >
              <div className="px-6 py-4 space-y-4">
                ...
                <div className="pt-4 border-t border-white/10">
                  ...
                  <a href={resumeLink} target="_blank">
                    <Button className="w-full bg-primary hover:bg-primary/90 text-black transition-all rounded-lg relative group overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-r from-primary via-yellow-400 to-primary opacity-0 group-hover:opacity-100 animate-gradient-xy transition-opacity" />
                      <FileText className="w-4 h-4 mr-2 relative z-10" />
                      <span className="relative z-10">Resume</span>
                    </Button>
